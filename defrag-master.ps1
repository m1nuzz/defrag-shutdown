...
The generated 'defrag-now.ps1' requires administrative privileges to run Optimize-Volume.
...
$shutdownChoice = $shutdownChoice.Trim()

if ($shutdownChoice -ceq "Y") { # Case-sensitive check for 'Y'
    $doShutdown = $true
    ...
    Write-Host "PC will remain on option will be included in the execution script." -ForegroundColor Yellow
}
Write-Host ""

# --- Generate and Save Execution Script ---
Write-Host "Generating execution script '$ExecutionScriptFile'..." -ForegroundColor Cyan
...
# --- Build the string for the $DoShutdown parameter line ---
# Explicitly output "$true" or "$false" string, correctly formatted for the generated script
# Ensure we generate "$true" or "$false" literal, not $(...)
$doShutdownLiteral = if ($doShutdown) { "`$true" } else { "`$false" }
$doShutdownLine = "`$DoShutdown = $($doShutdownLiteral)"


# Construct the content of the execution script using a Here-String
...
$selectedDrivesInfoString # Insert the pre-built string for the array here
$doShutdownLine # Insert the pre-built shutdown line here
...
if (`$DoShutdown) {
    Write-Host "`nDefragmentation tasks complete. Shutting down in 10 seconds..." -ForegroundColor Yellow
...
try {
    $executionScriptContent | Set-Content -Path $ExecutionScriptFile -Force -Encoding UTF8
    ...
    Write-Error "Error generating or launching execution script '$ExecutionScriptFile': $($_.Exception.Message)"
}


Write-Host ""
Write-Host "Master script finished." -ForegroundColor Cyan
Read-Host "Press Enter to exit."
